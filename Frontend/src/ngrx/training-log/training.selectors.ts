...
export const trainingsForMonth = createSelector(trainings, (trainings, month) =>
  trainings.filter((training) => training.dateTrained)
);
...
export const trainingMedia = createSelector(selectTrainingState, (state) =>
  Object.values(state.media[state.selectedTrainingId] ?? {})
);
