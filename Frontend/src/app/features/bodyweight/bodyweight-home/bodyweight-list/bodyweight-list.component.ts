...
import {
  TableAction,
  TableConfig,
} from "src/app/shared/material-table/table-models/table-config.model";
...
import {
  ConfirmDialogConfig,
  ConfirmResult,
} from "src/business/shared/confirm-dialog.config";
import { CRUD } from "src/business/shared/crud.enum";
import { unitSystem } from "src/ngrx/auth/auth.selectors";
import {
  bodyweightDeleted,
  setSelectedBodyweight,
} from "src/ngrx/bodyweight/bodyweight.actions";
...
export class BodyweightListComponent implements OnInit, OnDestroy {
  ...
  private deleteDialogConfig = new ConfirmDialogConfig({
    title: "BODYWEIGHT.DELETE_TITLE",
  ...
  @ViewChild(MaterialTableComponent, { static: true })
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.store
        .select(unitSystem)
        .subscribe((system) => (this._unitSystem = system)),

      this.store.select(isMobile).subscribe((mobile) => {
  ...
  getTableConfig() {
    const tableConfig = new TableConfig({
      filterFunction: (data: Bodyweight, filter: string) => {
        return (
          transformWeight(data.value, this._unitSystem)
            .trim()
            .toLocaleLowerCase()
            .indexOf(filter) !== -1 ||
          moment(data.date)
  ...
  getTableColumns() {
    return [
      new CustomColumn({
        ...
        sort: true,
        displayFn: (item: Bodyweight) =>
          transformWeight(item.value, this._unitSystem),
      }),
      new CustomColumn({
        definition: "date",
        title: "SHARED.DATE",
  ...
  onSelect = (bodyweight: Bodyweight) =>
    this.store.dispatch(setSelectedBodyweight({ entity: bodyweight }));
  ...
  onUpdate(bodyweight: Bodyweight) {
    const dialogRef = this.uiService.openDialogFromComponent(
      BodyweightCreateEditComponent,
      {
        ...
        autoFocus: false,
        data: {
          title: "BODYWEIGHT.UPDATE_TITLE",
          action: CRUD.Update,
          bodyweight: Object.assign({}, bodyweight),
        },
        panelClass: ["dialog-container"],
      }
    );
  ...
  onDeleteSingle(bodyweight: Bodyweight) {
    this.deleteDialogConfig.message = this.translateService.instant(
      "BODYWEIGHT.DELETE_DIALOG",
      {
        bodyweight: transformWeight(bodyweight.value, this._unitSystem),
        date: moment(bodyweight.date).format("D, MMM"),
      }
    );
  ...
  onDeleteSelection(bodyweights: Bodyweight[]) {
    this.deleteDialogConfig.message = this.translateService.instant(
      "BODYWEIGHT.DELETE_SELECTION_DIALOG",
      { length: bodyweights.length }
    );
  ...
  deleteBodyweight(bodyweight) {
    this.bodyweightService.delete(bodyweight.id).subscribe(
      () => {
