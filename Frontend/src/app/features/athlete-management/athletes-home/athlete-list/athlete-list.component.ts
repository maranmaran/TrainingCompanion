...
import {
  athleteDeleted,
  setSelectedAthlete,
} from "src/ngrx/athletes/athlete.actions";
...
export class AthleteListComponent implements OnInit, OnDestroy {
  ...
  getTableConfig() {
    const tableConfig = new TableConfig({
      filterFunction: (data: ApplicationUser, filter: string) =>
        data.fullName.trim().toLocaleLowerCase().indexOf(filter) !== -1 ||
  ...
  getTableColumns() {
    return [
      new CustomColumn({
        ...
        definition: "avatar",
        title: "",
        displayFn: (item: ApplicationUser) =>
          `<img class="avatar-table-img avatar-img" src="${item.avatar}"/>`,
      }),
      new CustomColumn({
        definition: "fullName",
      ...
      new CustomColumn({
        ...
        componentInputs: (item: ApplicationUser) => {
  ...
  onSelect = (athlete: ApplicationUser) =>
    this.store.dispatch(setSelectedAthlete({ entity: athlete }));
  ...
  onUpdate(athlete: ApplicationUser) {
    const dialogRef = this.uiService.openDialogFromComponent(
      AthleteCreateEditComponent,
      {
        ...
        autoFocus: false,
        data: {
          title: "ATHLETE_MANAGEMENT.UPDATE_TITLE",
          action: CRUD.Update,
          athlete: athlete,
        },
        panelClass: ["dialog-container"],
  ...
  onDeleteSingle(athlete: ApplicationUser) {
    //TODO i18n
    this.deleteDialogConfig.message = `<p>Are you sure you wish to delete user ${athlete.firstName} ${athlete.lastName} ?</p>
    ...
    dialogRef
      ...
      .subscribe((result: ConfirmResult) => {
        if (result == ConfirmResult.Confirm) {
          this.userService.delete(athlete.id, AccountType.Athlete).subscribe(
            () => {
