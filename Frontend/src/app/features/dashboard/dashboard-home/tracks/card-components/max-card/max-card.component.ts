...
export class MaxCardComponent implements OnInit {
  ...
  title = this.translate.instant('DASHBOARD.CARDS.MAX_CARD_TITLE');
  ...
  onFetchData(params: CardParameters) {
    this.reportService
      .getDashboardMaxReport(this._userId, params.exerciseTypes.map(x => x.id), params.dateFrom, params.dateTo)
      .pipe(take(1))
      .subscribe(
  ...
  getChartConfigs(chartData: ChartData<number, Date>, params: CardParameters) {
    ...
    this.config = [
      GetMaxCardChartConfig(params.settings, chartData.dataSets, labels),
    ];
  }
}
